...
def main():  
    ...
    if len(sys.argv) <= 1:
        ...
        while key == -1:
            ...
            thresh, output = cv2.threshold(grayscale, 0, 255, cv2.THRESH_OTSU)
            ...
            chosenGroup = 255*(labelImage == 2).astype("uint8")
                        
            # Show the image
            cv2.imshow(windowName, grayscale)
            cv2.imshow("THRESH", output)
            cv2.imshow("GROUPS", colorGroups)
            cv2.imshow("CHOSEN", chosenGroup)
            #print("Threshold:", thresh)
            print("Groups:", groupCnt)
