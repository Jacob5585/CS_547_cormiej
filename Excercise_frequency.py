...
def on_mouse(event, x, y, flags, parm):
    global left_mouse_down
    global right_mouse_down

    def draw_to_mask_image(parm, x, y, fill_value=0.0):
        mask_image = parm[0]
        circle_radius = parm[1]
    ...
    elif event == cv2.EVENT_LBUTTONDOWN or (left_mouse_down and event == cv2.EVENT_MOUSEMOVE):
        left_mouse_down = True
        draw_to_mask_image(parm, x, y, fill_value=0.0)
        
    elif event == cv2.EVENT_RBUTTONDOWN or (right_mouse_down and event == cv2.EVENT_MOUSEMOVE):
        left_mouse_down = True
        draw_to_mask_image(parm, x, y, fill_value=0.0)
...
def compute_inverse_fourier(complex_image, nonzeroRows=0):
    return cv2.idft(complex_image, flags=cv2.DFT_REAL_OUTPUT, nonzeroRows=nonzeroRows)
...
def filter_with_fourier(image, kernel, mask_image=None):
    ...
    padded_kenerl = cv2.copyMakeBorder(fimage,
                                      0, padded_rows - fimage.shape[0], 
                                      0, padded_cols - fimage.shape[1], 
                                      borderType=cv2.BORDER_CONSTANT, 
                                      value=0)
...
def make_gaussian_filter(sidelen):
    guassianCol = cv2.getGaussianKernel(ksize=sidelen, sigma=0)
    guassianRow = cv2.transpose(guassianCol)
    kenerl = np.matmul(guassianCol, guassianRow)

    return kenerl

def do_frequency(image, mask_image):
    kenerl = make_gaussian_filter(81)
    output = filter_with_fourier(image, kenerl, mask_image)
